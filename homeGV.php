...
<div class="row">
	...
	if($kn){
		$bm=$_REQUEST['bm'];
		$sql="select * from user u join giangvien g on u.user_id=g.user_id where u.user_code='$bm'";
...
<br />
<h5 style="color:#F63; font-size:25px;">Khóa Học Giảng Dạy</h5>
</br>
...
?>
<br />
                    	<td><center><p style="background-image: url('https://tse3.mm.bing.net/th?id=OIP.KBCZQ_Mrn2I5jzatY3AFkwHaEK&pid=Api&P=0&h=180;" class="u border"><a href="cthpgv.php?bm=<?php 
						echo $_REQUEST['bm']?>&&ig=<?php echo $t['id_giangvien'] ?>&&ihp=<?php echo md5($tt['id_hocphan']); ?>&&il=<?php echo $tt['id_lophocphan'] ?>&&ctmh"><?php echo "<br/>"."<hp>".$tt['tenhocphan'].'&nbsp;('.$tt['mahocphan'].")"."</a></hp><br><br>Lớp Học Phần&nbsp;:&nbsp;" ?><?php echo $tt['tenlophocphan'] ?></p></center></td>
                    <?php
	$a++;
	if($a==4){
		echo "</tr>";
    ...
    <p></p>
            <center><?php include_once("Controller/cPageG.php"); ?></center>
            <br />
</div>
...
$ig=md5($t['id_giangvien']);
	...
	$sql1="select * from giangvien gv join giangday d on gv.id_giangvien=d.id_giangvien
	join monlop m on m.id=d.id join hocphan hp on hp.id_hocphan=m.id_hocphan
	join lophocphan l on l.id_lophocphan=m.id_lophocphan
	join ct_hocphan c on c.id_hocphan=hp.id_hocphan
	where md5(gv.id_giangvien)='$ig' or md5(d.id_giangvienTH1)='$ig' or md5(d.id_giangvienTH2)='$ig'  limit $start_from,$bangghimoitrang";
	$laymh1=mysql_query($sql1);
	$bangghimoitrang1=!empty($_GET['per_page'])?$_GET['per_page']:8;
	$tranghientai1=!empty($_GET['page'])?$_GET['page']:1;
	$pt1="select count(d.id_giangday) from giangvien gv join giangday d on gv.id_giangvien=d.id_giangvien
	join monlop m on m.id=d.id join hocphan hp on hp.id_hocphan=m.id_hocphan
	join lophocphan l on l.id_lophocphan=m.id_lophocphan
	join ct_hocphan c on c.id_hocphan=hp.id_hocphan
	where md5(gv.id_giangvien)='$ig' or md5(d.id_giangvienTH1)='$ig' or md5(d.id_giangvienTH2)='$ig' ";
	$qr1=mysql_query($pt1); 
...
?>
<br />
                    	<td><center><p style="background-image: url('https://tse3.mm.bing.net/th?id=OIP.KBCZQ_Mrn2I5jzatY3AFkwHaEK&pid=Api&P=0&h=180;" class="r"><a href="cthpgv.php?bm=<?php 
						echo $_REQUEST['bm']?>&&ig=<?php echo $t['id_giangvien'] ?>&&ihp=<?php echo md5($tt1['id_hocphan']); ?>&&il=<?php echo $tt1['id_lophocphan'] ?>&&ctmh"><?php echo "<br/>"."<hp>".$tt1['tenhocphan'].'&nbsp;('.$tt1['mahocphan'].")"."</a></hp><br><br>Lớp Học Phần&nbsp;:&nbsp;" ?><?php echo $tt1['tenlophocphan'] ?></p></center></td>
                    <?php
	$a1++;
	if($a1==4){
		echo "</tr>";
    ...
    <p></p>
            <center><?php include_once("Controller/cPage2.php"); ?></center>
            <br />
</div>
